...
class NikkiUtil:
    ...
    def map_jump(self, coordinates, destination="花焰群岛", screenshot_path="map"):
        """
        地图传送
        ...
        self.to_main_menu()  # 确保在主菜单界面
        self.press_keyboard('m')
        if not self.wait_image("return"):
            self.press_keyboard('m')

        # 扩大四次地图，缩小一次地图
        self.click_coordinate(376, 1037)
        self.click_coordinate(376, 1037)
        self.click_coordinate(376, 1037)
        self.click_coordinate(376, 1037)
        self.click_coordinate(60, 1040)

        self.click_coordinate(1700, 100) # 打开地图列表

        time.sleep(1)

        # 打开目的地destination
        self.map_jump_chose_position(destination, screenshot_path)
        ...
        self.wait_and_click_image('teleport', max_attempts=5)

        self.wait_main_menu()

    def map_jump_chose_position(self, destination, screenshot_path):
        """
        位置点击——地图专用
        :param destination:
        ...
        if not target_pos:
            self.map_jump_chose_position("心愿原野", screenshot_path)
            # 鼠标移动到（1555，555）
            pyautogui.moveTo(1555, 555)
            # 鼠标滚轮向下滑动1000
            pyautogui.scroll(-10000)
        ...
        if target_pos:
            ...
            self.click_coordinate(*target_pos)

    def activate_window_by_title(self, window_title="无限暖暖"):
        """
        根据窗口标题激活窗口
    ...
    def activate_window_by_title_force(self, window_title="无限暖暖"):
        """
        根据窗口标题激活窗口
        :param window_title: 窗口标题
        """
    ...
    def wait_images(self, image_paths, wait_interval=0.5, max_attempts=100):
        """
    ...
    def wait_and_click_image(self, image_path, wait_interval=1, max_attempts=15, press_time=0.1):
        """
        在屏幕上查找图片并点击，同时确保指定窗口已激活。
        :param image_path: 图片路径
        :param wait_interval: 等待间隔时间
    ...
    def click_coordinate(self, x, y):
        ...
        :param y: 纵坐标
        """
        # self.activate_window_by_title()  # 激活目标窗口
        time.sleep(0.1)
        pyautogui.mouseDown(x, y)  # 按下鼠标
        time.sleep(0.2)
        pyautogui.mouseUp(x, y)  # 松开鼠标
    ...
    def press_keyboard(self, key, duration=0.1):
        """
        模拟按下键盘上的某个键。
        :param key: 要按下的键
    ...
    def to_main_menu(self):
        ...
        for i in range(15):
            ...
            if result:
                ...
                # if(self.wait_image("return",max_attempts=1)):
                #     self.wait_and_click_image("return")
                # else:
                self.click_coordinate(70,55)
                self.click_coordinate(70,55)
                self.click_coordinate(70,55)
                self.click_coordinate(70,55)
                self.click_coordinate(70,55)
    ...
    def wait_main_menu(self, max_retries=100):
        """
        等待主页面加载完毕。
        通过不断查找并点击"return"按钮，直到找到"daMiao"图像为止。
        """
    ...
    def close_game_window(self, window_title="无限暖暖"):
        # 尝试根据标题关闭窗口
        try:
